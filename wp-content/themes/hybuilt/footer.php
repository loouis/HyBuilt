		...
		<?php /* if(!is_front_page()): */?>
			...
			<footer class="footer" role="contentinfo">
				...
				<div class="wrap">
					...
					<?php $footerLogo = get_field('hybuilt_logo_white', 'option');?>
					<div class="logo">
		...
		<nav class="main-navigation" role="navigation">
			...
			<?php
				...
				$d_navImage = wp_get_attachment_image_src($navId, '');
				$navLink = get_sub_field('nav_link', 'option');
			?>

			
			<a href="<?php echo $navLink;?>" class="lay-split main-navigation__link">			

				<div class="text">
					<h3 class="boldest uppercase f-white f-shadow--dark"><?php the_sub_field('nav_link-name', 'option');?></h3>
        </div>

        <?php if( wp_is_mobile() ){ ?>
          <div class="image cover" style="background: black url(<?php echo $m_navImage[0]; ?>)"></div>
        <?php }else{ ?>
          <div class="image cover" style="background: black url(<?php echo $d_navImage[0]; ?>)"></div>
        <?php } ?>
    ...
    <script async src="https://www.googletagmanager.com/gtag/js?id=UA-114892634-1"></script>
    <script>
      window.dataLayer = window.dataLayer || [];
      function gtag(){dataLayer.push(arguments);}
      gtag('js', new Date());

      gtag('config', 'UA-114892634-1');
    </script>
